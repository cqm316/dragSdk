(function(window,document){
    ...
    Drag.prototype.init = function(){
        ...
        self.fnDragHtml(this._options.leftSideId,this._options.rightSideId,this._options.pdfImgs)
        self.fnSetDragAttr();
        self.fnGetDrag(cls);
    };
    //渲染组件页面
    Drag.prototype.fnDragHtml = function(leftSideId,rightSideId,pdfImgs){
        var self = this
        var o = document.body;
        var leftHtml = document.getElementById(leftSideId);
        var rightHtml = document.getElementById(rightSideId);
        var wrap = document.createElement("div");
        var pdfImgs = pdfImgs ? pdfImgs : []
        wrap.setAttribute('class','drag-container');

        var dHtml = ['<div class="drag-btn-sub" id="btnDragSub">' +
            '            <span>立即签署</span>' +
            '            </div>' +
        ...
        o.appendChild(wrap);
        if(leftSideId) document.getElementById('dragLeftSide').appendChild(leftHtml);
        if(rightSideId) document.getElementById('dragRightSide').appendChild(rightHtml);
        if(pdfImgs){
            var imgHtml = ''
